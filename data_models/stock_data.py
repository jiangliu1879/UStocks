...
class StockData:
    ...
    """
    def __init__(self, stock_code: str, timestamp: str, open: float, high: float, low: float, close: float, volume: int, turnover: float, id: Optional[int] = None, description: Optional[str] = None):
        self.id = id
    ...
    def save(self) -> bool:
        ...
        try:
            ...
            INSERT INTO stock_data (
                stock_code, timestamp, open, high, low, close, volume, turnover, description
            ) VALUES (
                %s, %s, %s, %s, %s, %s, %s, %s, %s
            )
            ...
            logger.error(f"[StockData::save] 保存股票数据时出错: {e}", exc_info=True)
            db_manager.close_connection(connection)
            return False

    @classmethod
    def batch_save(cls, data_list: List['StockData'], batch_size: int = 1000) -> int:
        """
        批量保存多条数据到数据库
        ...
        try:
            ...
            logger.info(f"[StockData::batch_save] 批量保存股票数据完成，共保存 {saved_count} 条记录")
            return saved_count
            
        except Error as e:
            logger.error(f"[StockData::batch_save] 批量保存股票数据时出错: {e}", exc_info=True)
            connection.rollback()
            db_manager.close_connection(connection)
            return 0
    ...
    def get_stock_codes(cls) -> List[str]:
        ...
        try:
            ...
            sql = "SELECT DISTINCT stock_code FROM stock_data ORDER BY stock_code ASC"
            cursor.execute(sql)
            ...
            logger.error(f"[StockData::get_stock_codes] 获取股票代码列表时出错: {e}", exc_info=True)
            db_manager.close_connection(connection)
            return []

    @classmethod
    def query(cls, conditions: Dict, limit: Optional[int] = None, order_by: Optional[str] = None) -> List['StockData']:
        """
        根据条件查询多条记录
        ...
        try:
            ...
            logger.error(f"[StockData::query] 查询股票数据时出错: {e}", exc_info=True)
            db_manager.close_connection(connection)
            return []
    ...
    def delete_by_stock_code(cls, stock_code: str) -> int:
        ...
        try:
            ...
            cursor.close()
            db_manager.close_connection(connection)
            logger.info(f"[StockData::delete_by_stock_code] 已删除 {stock_code} 共 {deleted} 条记录")
            return deleted
        except Error as e:
            logger.error(f"[StockData::delete_by_stock_code] 删除时出错: {e}", exc_info=True)
            connection.rollback()
            db_manager.close_connection(connection)
            return 0